...
// Fetch only the latest unread notification
            ...
            // Fetch published announcements ordered by newest
            $stmt = $pdo->query("SELECT * FROM announcements WHERE status = 'published' ORDER BY created_at DESC LIMIT 3");
            $latest_announcements = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ?>

            <div class="notification-wrapper" style="position: relative;">
                <img src="images/bell.jpg" alt="Notifications" class="icon-bell" id="notif-bell"
        ...
        <div class="user-dropdown" style="position: relative; display: inline-block;">
            ...
            <div id="user-menu" class="dropdown-content"
                ...
                <a href="logout.php"
                    style="display: block; padding: 8px 16px; text-decoration: none; color: black;">Logout</a>
            </div>
        </div>
    ...
    <div class="layout">
        <aside class="sidebar">
            <ul>
                <li class="active">
                    <img src="images/finalhome.png" alt="Home">
                    <span>Home</span>
                </li>
                <li>
                    <img src="images/finaluser.png" alt="Profile">
                    <span><a href="profile.php" style="color: black;text-decoration: none;">My Profile</a></span>
                </li>
                <li>
                    <img src="images/finalsave.png" alt="Saved">
        ...
        <div class="card">
            ...
            <ul class="checkbox-list">
                ...
                <li><input type="checkbox" class="institute-filter" value="IC"> Institute of
                    Computing</li>
                <li><input type="checkbox" class="institute-filter" value="ITed"> Institute of Leadership
                    Entrepreneurship, and Good Governance</li>
                <li><input type="checkbox" class="institute-filter" value="Institute of Teacher Education"> Institute of
                    Teacher Education</li>
                <li><input type="checkbox" class="institute-filter" value="Institute of Aquatic and Applied Sciences">
                    Institute of Aquatic and Applied Sciences</li>
            </ul>
        </div>

        <!-- Sort Options -->
        <div class="card">
            <h2 class="card-title">Sort Articles By</h2>
            <ul class="sort-options">
                <li><input type="radio" name="sort-option" class="sort-filter" value="newest" checked> Newest First</li>
                <li><input type="radio" name="sort-option" class="sort-filter" value="oldest"> Oldest First</li>
                <li><input type="radio" name="sort-option" class="sort-filter" value="most_likes"> Most Likes</li>
                <li><input type="radio" name="sort-option" class="sort-filter" value="most_comments"> Most Comments</li>
            </ul>
        </div>
...
// Let's add a fallback if user has no uploaded profile picture
            ...
            <form class="post-input" action="submit_article.php" method="POST">
                <input type="text" class="post-input1" id="articleInput" name="title"
                    ...
                    <div class="form-container">
                        ...
                        <form action="submit_article.php" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group full-width">
                                ...
                                <textarea type="text" name="abstract" rows="3"></textarea>
                            </div>
...
';
                    ...
                    echo '  <div class="post-content">' . $row['abstract'] . '</div>';
    ...
    <script>
        ...
        function updateFeed() {
            const selectedInstitutes = Array.from(document.querySelectorAll('.institute-filter:checked'))
                .map(cb => cb.value);

            const selectedSort = document.querySelector('.sort-filter:checked').value;
            ...
            fetch('filter_feed.php', {
                ...
                .then(data => {
                    ...
                    <div class="post-header">
                        <img class="avatar" src="uploads/profile_pictures/${article.profile_picture || 'default_profile.png'}" alt="User">
                        <div>
                            <strong>${article.full_name}</strong><br>
                            <span class="post-date">${new Date(article.created_at).toLocaleString('en-US', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                            hour: 'numeric',
                    ...
                    <div class="post-title"><strong>${article.title}</strong></div>
                    <div class="post-content">${article.abstract || ''}</div>`;

                        // Article image
                        let imageHTML = '';
                        if (article.featured_image) {
                            imageHTML = `<div class="post-image"><img src="${article.featured_image}" alt="Article Image" class="responsive-img"></div>`;
                        }

                        // Article actions
                        let actionsHTML = `
                    <div class="post-actions">
                        <button class="like-btn" data-article-id="${article.id}">👍 <span class="like-count" id="like-count-${article.id}">${article.likes}</span></button>
                        <button class="comment-btn">💬 <span class="comment-count" id="comment-count-${article.id}">${article.comments}</span></button>
                    </div>
                    <div class="post-comments">
                        ...
                        // Combine all parts
                        articleCard.innerHTML = dropdownHTML + headerHTML + imageHTML + actionsHTML;
                        postFeed.appendChild(articleCard);
                    });
        ...
        document.querySelectorAll('.institute-filter').forEach(checkbox => {
            checkbox.addEventListener('change', updateFeed);
        });

        document.querySelectorAll('.sort-filter').forEach(radio => {
            radio.addEventListener('change', updateFeed);
        });
                        ...
                        fetch('post_comment.php', {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    newComment.innerHTML = `<div class="comment-text"><strong>${data.commenter_name}:</strong> ${commentText}</div>`;
